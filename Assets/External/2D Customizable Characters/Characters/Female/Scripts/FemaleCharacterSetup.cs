...
[ExecuteInEditMode]
public class FemaleCharacterSetup : MonoBehaviour {
    ...
    void Awake()
    {
        if (downDirection != null)
        {
            ...
            if (cachedDownSpriteRenderers != null && cachedDownSpriteRenderers.Length > 0)
            {
                ...
                makeup = GetSpriteRendererBySlotName(cachedDownSpriteRenderers, "__makeup slot").sprite;
				makeupColor = GetSpriteRendererBySlotName(cachedDownSpriteRenderers, "__makeup slot").color;
    ...
    private void UpdateSpritesForDirection(SpriteRenderer[] cachedSpriteRenderers, string direction, bool updateColorOnly)
    {
        if (cachedSpriteRenderers != null && cachedSpriteRenderers.Length > 0)
        {
			...
			eyesSR.color = new Color (eyeColor.r, eyeColor.g, eyeColor.b, eyesSR.color.a);

			SpriteRenderer eyeshapeSR = GetSpriteRendererBySlotName(cachedSpriteRenderers, "eyes");
			eyeshapeSR.color = new Color (255, 255, 255, eyeshapeSR.color.a);
			UpdateSprite(eyeshapeSR, eyeShape, eyeshapeSR.color, direction, isMainDirection, updateSprites);
            ...
            SpriteRenderer makeupSlotSR = GetSpriteRendererBySlotName(cachedSpriteRenderers, "__makeup slot");
			UpdateSprite(makeupSlotSR, makeup, makeupColor, direction, isMainDirection, updateSprites);
